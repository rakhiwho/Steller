...
function Explore() {
  ...
  return (
    <>
      <div
        className={` ${
          !darkTheme ? " absolute top-[7.4%] bg-indigo-900 text-slate-300" : "bg-white"
      ...
      >
        <div
          ...
          } flex flex-row justify-between fixed pt-3  z-[99] h-fit  content-center w-[80vw] text-xl text-indigo-900 `}
        >
          <button className=" ml-4 text-white rounded-full bg-indigo-700 p-2 mr-2">
            <CiBrightnessDown className="size-4" />
          </button>
          <p className="flex-1 m  text-xl ">Explore</p>
          <button
            onClick={() => navigate("/explore/search")}
            className=" mx-3 text-white rounded-full bg-indigo-700 p-2"
          >
            <IoIosSearch />
          </button>
        </div>
