...
Comments...
  ...
  <div className={` ${!darkTheme ?'bg-indigo-900':''} all_comments p-3`} >
...
}
{
 ...
 <input value={content} className={`${!darkTheme ?'bg-indigo-950':''}  text-slate-300 `} onChange={(e)=>setContent(e.target.value)}  type="text" name="" id="" /><IoMdSend className={`${!darkTheme ?'bg-indigo-950 text-slate-300 ':''} icon-send`} onClick={handleSubmit} />
   </div>
  </div>
  </>
  )
