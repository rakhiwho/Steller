 ...
 const [action , setAction] = useState(false);
  ...
  return (
    <>
      ...
      {!loading && (
        <div
          className={` ${deleting ? 'blur-lg':''} ${!darkTheme ?'hover:shadow-2xl':'post_Container'}   post_container overflow-hidden h-fit lg:w-[17.2vw]  relative mb-0 `}
        >
          <div {...Handlers} className={`${!darkTheme ?'bg-indigo-900':''} post-img mt-0`}>
            <div className={`${!darkTheme ?'bg-indigo-900':''} userName ml-0 `}>
              <p className="text-xs owner ml-0 mr-4 self-center  ">{owner2?.userName || 'loading'}</p>
            ...
            <div className={` ${!darkTheme ?'bg-indigo-900':''} discription_of_img`}>
...
{/* .........................three dot........................... */}
                ...
                <div
                  ...
                  onMouseOut={() => setThreedot(false)}
                  className={`
                  ${  threedot ? " absolute right-3 text-xs bg-white " : "hidden"} ${!darkTheme ?'bg-indigo-950 text-slate-300':''}`
                  }
                >
                  <li
                    onClick={()=>setDeleting(true)}
                    className={`${!darkTheme ?'bg-indigo-950 text-slate-300':''}list-none flex flex-row p-2 mr-1 w-full`} 
                  >
                  ...
                  <li
                    ...
                    className={`${!darkTheme ?'bg-indigo-950 text-slate-300':''} w-full list-none flex flex-row p-2 mr-1`} 
                  >
   ...
   {/* ..........edit.............................. */}
              {!editing ? (
                <p className={` ${!darkTheme ?'bg-indigo-900':''} flex-wrap lg:text-sm text-xs font-medium   mb-2`}>
                  <ReadMore
                ...
                <>
                  <input
                    ...
                    className={` ${!darkTheme ?'bg-indigo-950 text-slate-300':''} input h-5 w-[full] border-none m-0 px-0 ml-1`}
                    type="text"
                  />
                  <input
                    value="save"
 ...
 { action &&    <div> <MessagePop text='changes done successfully' /></div>}
 { deleting &&    <div className={`  bg-white text-indigo-950 rounded-xl fixed top-[44%] flex flex-col h-[15vh] w-[25vw] `} >
                        <p className="py-4 pl-3">delete selected post ?</p>
                        <div className="flex flex-row p-2">

                        <button className="mx-5 px-3 rounded-md bg-indigo-950 text-slate-300" onClick={()=>setDeleting(false)} >No</button>
                        <button className="border-x-2 border-y-2 border-indigo-950 text-indigo-950 rounded-md px-3 hover:bg-indigo-950 hover:text-white" onClick={deleted}>Yes</button>
                        </div>
                      </div>}
    </>
  );
